...
If you still wish to use the material symbols font icons with `<Icon/>` you must attach the component `<LoadMaterialSymbols/>` in the `<head>`, and ensure you use either `filled` or `useFontIcon` to enable material symbols.
