...
As of version `1.40.0`, icons are now directly shipped from the package, and importing them directly is the recommended usage. See the [icons documentation](/resources/icons/) for the full list.

If you still wish to use the material symbols font icons with `<Icon/>` you must attach the component `<LoadMaterialSymbols/>` in the `<head>`, and ensure you use either `filled` or `useFontIcon` to enable material symbols. Relevant information on this can be found in the [Icon Component](/components/library/icon/react/) documentation.
