...
**This library should only be imported for departments actively working with the core design system team during its development. Do not use this library if you are not part of this development process, and contact the design system team instead if you are interested in its use.**
...
The Government of Ireland Design System uses the [Lato](https://fonts.google.com/specimen/Lato) font family via [`@fontsource/lato`](https://fontsource.org/fonts/lato). Lato fonts are included in `styles.css` — no additional font import is needed. Your bundler (Vite, webpack, etc.) resolves the font imports and bundles the font files automatically.
...
For Next.js applications, we recommend using [next/font/google](https://nextjs.org/docs/app/getting-started/fonts) instead, as it provides optimal font loading with automatic optimisation.
...
As of version <Tag type="info" text="1.40.0"/>, icons are now directly shipped from the package, and importing them directly is the recommended usage. See the [icons documentation](/resources/icons/) for the full list.

If you still wish to use the material symbols font icons with `<Icon/>` you may use the prop `useFontIcon` to enable this fallback. The CSS file is also provided within the package for ease of use.

```tsx
import '@ogcio/design-system-react/material-symbols.css'
...
<Icon icon="thumb_up" useFontIcon>
```

<Blockquote>
**NOTE:** There was a breaking change in **Next.JS 16 (Turbopack)**. Instead of loading the css file, you must attach the component `<LoadMaterialSymbols/>` in the `<head>`. Relevant information on this can be found in the [Icon Component](/components/library/icon/react/) documentation.
</Blockquote>
