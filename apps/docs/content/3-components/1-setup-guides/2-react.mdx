...
<Tabs id="fontLoadingTabs">
  <TabList>
    <TabItem value="nextjs" checked>Next.js (Recommended)</TabItem>
    <TabItem value="other">Other Frameworks</TabItem>
  </TabList>
  <TabPanel value="nextjs">
    **For Next.js applications, we strongly recommend using [`next/font/google`](https://nextjs.org/docs/app/getting-started/fonts)** as it provides optimal font loading with automatic optimization.
    ...
    <Blockquote>
      **Note:** The `LoadMaterialSymbols` component loads Material Symbols icons only. It does NOT load the Lato font, you must load fonts using `next/font/google` as shown above.
    </Blockquote>

  </TabPanel>
  <TabPanel value="other">
    For non-Next.js applications, you can load fonts using Google Fonts CDN (not recommended for production due to performance implications):
