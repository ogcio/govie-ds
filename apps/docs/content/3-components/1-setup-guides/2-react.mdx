...
For applications that need explicit control over font loading, a standalone `@govie-ds/fonts` package is available. This provides a `fonts.css` file with `@font-face` declarations that work across any framework (React, Angular, plain HTML, etc.).
