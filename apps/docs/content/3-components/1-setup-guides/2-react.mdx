...
```json
"dependencies": {
   "@govie-ds/theme-govie": "0.6.0",
   "@govie-ds/react": "0.34.0"
```
...
To support multiple languages across your application, we use the `initI18n` utility provided by `@govie-ds/react`. This ensures consistency, accessibility, and localisation across all components.
