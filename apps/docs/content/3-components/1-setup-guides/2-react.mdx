...
<Alert variant="danger">
  **Deprecated**: Loading Material Symbols is now no longer necessary and can be
  safely removed. If material symbols are absolutely necessary, please refer to
...
In cases where material symbols font library is necessary, you can place this import line within your style sheet. To use it with `<Icon/>` you must include the prop `useFontIcon`.

<ComponentContainer
  fullWidth
  className="min-w-0 [&_pre]:whitespace-pre-wrap [&_pre]:break-all [&_pre]:overflow-x-hidden"
>
  ```css @import
