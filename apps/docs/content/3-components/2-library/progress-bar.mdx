---
title: Progress Bar
description: Progress Bar HTML Component
status: in-review
libraries:
  - platform: 'figma'
    link: 'https://www.figma.com/design/ZEqfpXhocdZs1K5YfsE5ws/Design-system?node-id=4393-5943'
    status: 'stable'
  - platform: 'global'
    link: '?path=/docs/indicators-progressbar--docs'
    status: 'stable'
  - platform: react
    link: '?path=/docs/indicators-progressbar--docs'
    status: 'stable'
properties:
  - name: ProgressBarProps
    fields:
      - name: 'value'
        ofType: 'number'
        description: 'The current value of the progress bar.'
        required: true
      - name: 'max'
        ofType: 'number'
        description: 'The maximum value of the progress bar.'
        required: false
      - name: 'isIndeterminate'
        ofType: 'boolean'
        description: 'When true, sets the progress bar to an indeterminate state with an animation.'
        required: false
      - name: 'label'
        ofType: 'string'
        description: 'Custom label text to display alongside the progress bar.'
        required: false
      - name: 'size'
        ofType: 'string'
        description: 'Size of the progress bar. Options: sm, md, lg.'
        required: false
---

# Progress Bar

Use the progress bar component to visually represent the progress of a task or operation.

## Variants

### Progress Bar

<ComponentContainer>
  <ProgressBar value="40" max="100" size="sm" />
</ComponentContainer>

<Tabs id="tab1">
  <TabList>
    <TabItem value="tab11" checked>HTML</TabItem>
    <TabItem value="tab13">React</TabItem>
  </TabList>
  <TabPanel value="tab11">
    ```html
      <div class="gi-progress-bar-container">
        <div role="progressbar" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100" aria-labelledby="progress-bar-:S1:" aria-label="Progress bar" class="gi-progress-bar gi-h-2">
          <div class="gi-h-2 gi-bg-gray-700" style="width: 50%;">
          </div>
        </div>
      </div>
    ```
  </TabPanel>
  <TabPanel value="tab13">
    ```html
      import { ProgressBar } from '@govie-ds/react';

      <ProgressBar value={50} />
    ```

  </TabPanel>
</Tabs>

### Indeterminate Progress Bar

<ComponentContainer>
  <ProgressBar isIndeterminate="true" label="Loading..." />
</ComponentContainer>

<Tabs id="tab2">
  <TabList>
    <TabItem value="tab21" checked>HTML</TabItem>
    <TabItem value="tab23">React</TabItem>
  </TabList>
  <TabPanel value="tab21">        
    ```html
    <div class="gi-progress-bar-container">
      <div role="progressbar" aria-label="Progress bar" class="gi-progress-bar gi-h-2">
        <div class="gi-h-2 gi-bg-gray-700 gi-progress-bar-indeterminate"></div>
      </div>
      <span>Loading...</span>
    </div>
    ```
  </TabPanel>
  <TabPanel value="tab23">
    ```html
      import { ProgressBar } from '@govie-ds/react';

      <ProgressBar isIndeterminate label="Loading..." />
    ```

  </TabPanel>
</Tabs>

## Usage

<ComponentPropertiesTable componentId="progress-bar" />

### Progress Bar Props

- **value** (required): The current value of the progress bar.
- **max** (required): The max value of the progress bar.
- **isIndeterminate** (optional): Set infinite progress for the progress bar. This makes the progress bar indeterminate.
- **label** (optional): Set a custom label for the progress bar.

### Best Practices

- Ensure progress bars are visually distinct and easy to interpret against their background for clarity.
- Place progress bars close to the task or content they represent to maintain contextual relevance.
- Provide additional textual feedback (e.g., percentage complete or estimated time) to improve user understanding, especially for long-duration tasks.

### When to use this component

- Use a determinate progress bar when the progress of a task can be measured (e.g., file upload, form completion).
- Use an indeterminate progress bar when the duration of a task is unknown but progress should be indicated (e.g., loading state).

### When not to use this component

- Avoid using a progress bar for static values that do not visually indicate progress.
- Do not use a progress bar if the task completion time is very short, as it might appear unresponsive to the user.

### Accessibility

- Always include an appropriate label for screen readers to describe the task being performed.
- For determinate progress, ensure the current value and total value are conveyed to assistive technologies.

## Status

<ComponentStatusBlock componentId="progress-bar" />
