...
- **ref** (optional): A reference to the text area element.
- **rows** (optional): The number of visible text lines for the text area.
- **cols** (optional): The visible width of the text area in characters.
- **autoComplete** (optional): Specifies whether the browser should automatically complete the input.
- **maxChars** (optional): The maximum number of characters allowed in the text area.
- **halfFluid** (optional): If `true`, the text area will take up half the width of the container.
- **Other TextareaHTMLAttributes** (optional): The component also accepts other standard HTML textarea attributes, such as `placeholder`, `disabled`, `readOnly`, etc., through the `TextareaHTMLAttributes` generic props.
