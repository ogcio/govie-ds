---
title: Icon Button
description: Icon Button HTML Component
status: in-review
libraries:
  - platform: 'figma'
    link: 'https://www.figma.com/design/ZEqfpXhocdZs1K5YfsE5ws/Design-system?node-id=1386-7470&t=TDHF1QkZpdPfgn8m-4'
    status: 'stable'
  - platform: 'global'
    link: '?path=/docs/form-iconbutton--docs'
    status: 'stable'
  - platform: 'react'
    link: '?path=/docs/form-iconbutton--docs'
    status: 'stable'
component:
  id: 'icon-button'
  properties:
    - name: IconButtonProps
      fields:
        - name: 'icon'
          ofType: 'object'
          description: 'Specifies the Icon component that the button will accept. The value should be an object with an icon property.'
          required: true
        - name: 'variant'
          ofType: 'string'
          description: 'Defines the variant style of the button. Options: primary, secondary, flat.'
          required: false
        - name: 'appearance'
          ofType: 'string'
          description: 'Specifies the visual appearance style of the button. Options: default, dark, light.'
          required: false
        - name: 'size'
          ofType: 'string'
          description: 'Defines the size of the button. Options: medium, small, large.'
          required: false
        - name: 'disabled'
          ofType: 'boolean'
          description: 'Specifies whether the button is disabled.'
          required: false
        - name: 'onClick'
          ofType: 'function'
          description: 'A callback function that is triggered when the button is clicked.'
          required: false
---

# Icon Button

An Icon Button is a button that uses an icon instead of text to represent an action, like a thumbs-up for "like" or a trash can for "delete." It’s commonly used in places where space is limited, such as toolbars or mobile interfaces.

Icon buttons are ideal for quick, easily recognised actions, but they rely on familiar icons to avoid confusion. They work best when users can immediately understand the action without additional labels.

## Variants

### Default

<ComponentContainer>
  <IconButton
    icon={{
      icon: 'thumb_up',
    }}
    size="large"
  />
</ComponentContainer>

<Tabs id="tab3">
  <TabList>
    <TabItem value="tab31" checked>HTML</TabItem>
    <TabItem value="tab33">React</TabItem>
  </TabList>
  <TabPanel value="tab31">
    ```html
    <button
      data-element="icon-button-container"
      data-module="gieds-icon-button"
      class="gi-btn gi-btn-primary gi-icon-btn-regular">
      <span
        role="presentation"
        class="
          material-icons
          gi-block
          gi-text-[16px]">
        thumb_up
      </span>
    </button>
    ```
  </TabPanel>
  <TabPanel value="tab33">
    ```html
    import { IconButton } from "@govie-ds/react";
    
    <IconButton
      icon={{
        icon: 'thumb_up'
      }}
      onClick={() => {}}
    />
    ```
  </TabPanel>
</Tabs>

## Usage

<ComponentPropertiesTable componentId="icon-button" />

### Icon Button Props

- **icon** (optional): Specifies the Icon component that the button will accept.

  - The value should be a string representing the icon's identifier or name.
  - Example: `edit`, `delete`

- **variant** (optional): Defines the variant style of the button.

  - Options: `primary`, `secondary`, `flat`
  - The control for this prop is a string, allowing the user to set the button variants.

- **appearance** (optional): Specifies the visual appearance style of the button.

  - Options: `default`, `dark`, `light`
  - The control for this prop is a string, allowing the user to set the button's appearance.

- **size** (optional): Defines the size of the button.

  - Options: `medium`, `small`, `large`
  - The control for this prop is a string, allowing the user to set the defined button sizes.

- **disabled** (optional): Specifies whether the button is disabled.

  - The control for this prop is a boolean, allowing the user to enable or disable the button.

### Best Practices

### When to use this component

- **Quick Actions**: Use the `Icon Button` component for actions that are represented clearly by an icon alone, such as a thumbs-up for liking, a heart for favoriting, or a star for bookmarking.
- **Limited Space**: Ideal for UI areas with limited space where a standard button with text would take up too much room, such as toolbars, navigation bars, or compact action panels.
- **Repetitive Actions**: Use when an action is repeated across many items in a list (e.g., like buttons for each post in a feed), where text on every button would be visually overwhelming.
- **Visual Emphasis on Iconography**: Best for interfaces where icons are central to the design, helping users quickly recognise actions through familiar visuals rather than text.

### When not to use this component

- **Ambiguous Actions**: Avoid using an `Icon Button` when the icon alone doesn’t clearly convey the intended action. In such cases, a text label or a standard button may be more effective.
- **Primary or Complex Actions**: For primary actions or complex tasks requiring explanation, consider a standard button with text or an icon and text combination to avoid confusion.
- **Accessibility Concerns**: When accessibility is critical, ensure the icon is universally recognisable, or consider using a button with both icon and text. Avoid using `Icon Button` components in isolation if screen readers or visually impaired users may have difficulty interpreting the icon's function.

## Status

<ComponentStatusBlock componentId="icon-button" />
