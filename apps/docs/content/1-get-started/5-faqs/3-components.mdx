...
Component implementations are provided that includes vanilla [HTML components](/components/get-started/html/) that are **local** and stable and an alpha library of [React components](/components/get-started/react/) currently under development.
...
The use of macros that are [Nunjucks](https://mozilla.github.io/nunjucks/) and [Jinja](https://jinja.palletsprojects.com/) compatible means that consumers of the global HTML components are protected from any underlying HTML markup changes whilst the global components are being built or as new features are added.
...
However, we will still support the local components as they are globalised, providing assistance and implementing any necessary bug fixes.
...
There will be some situations where the use of the HTML markup directly will be necessary, either due to a lack of server environment or templating tooling.
