...
Component implementations are provided that includes vanilla [HTML components](/components/setup-guides/html/) and [React components](/components/setup-guides/react/).

</Faq>

<Faq question="What are local components?">

The existing [HTML components](/components/setup-guides/html/) are described as **local** because they are tailored for usage within a gov.ie context. The components are styled for specific use cases, so may not be suitable for your needs.
...
The existing [HTML component](/components/setup-guides/html/) implementations are undergoing the following globalisation process, converting a single HTML component at a time from local to global based on priority:
...
The [HTML components](/components/setup-guides/html/) are **data-driven** components (also known as **attribute-driven** components).
