...
import { ReactNode, FieldsetHTMLAttributes, Children, createContext, useContext } from 'react';
...
const FormField = (props: FormFieldProps) => {
  ...
  if (isLegacy) {
    console.warn('[FormField] Using legacy props. Please migrate to the new composable API.');
...
const FormFieldBase = ({ children, className, ...props }: FormFieldBaseProps) => {
  const allChildren = Children.toArray(children);

  const label = allChildren.find((child) => getSpecialComponentType(child) === 'FormFieldLabel');
  const hint = allChildren.find((child) => getSpecialComponentType(child) === 'FormFieldHint');
  const error = allChildren.find((child) => getSpecialComponentType(child) === 'FormFieldError');
  const rest = allChildren.filter(
    (child) => !isSpecialComponent(child, ['FormFieldLabel', 'FormFieldHint', 'FormFieldError']),
  );
