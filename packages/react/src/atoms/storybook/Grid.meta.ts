...
export const gridMeta = {
  ...
  argTypes: {
    container: {
      control: false,
      description: 'When true, activates grid container mode with Figma-responsive defaults. When false, the element acts as a grid item.',
      table: {
        type: {
          summary: 'boolean'
        },
    ...
    role: {
      ...
      table: {
        type: {
          summary: '"region" | "navigation" | "complementary" | "search" | "form"'
        }
      }
    },
    ariaLabel: {
      control: false,
      description: 'Accessible label for the container. Use when the grid has a `role` prop to provide an accessible name for the landmark. Maps to `aria-label`.',
      table: {
        type: {
          summary: 'string'
        }
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Grid is a flex-based layout component with two modes: container (distributes children according to Figma column spec by default) and item (controls column span). Supports responsive breakpoint objects for columns, gap, and size.'
      }
    }
  }
};
