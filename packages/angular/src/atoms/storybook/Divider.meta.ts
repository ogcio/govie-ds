...
export const dividerMeta = {
  ...
  argTypes: {
    ...
    styles: {
      ...
      description: 'Inline styles applied directly to the element. Use for truly dynamic values that cannot be expressed as Tailwind classes.',
      table: {
        type: {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Divider is a visual separator rendered as a native `<hr>`. It has no margin — spacing is controlled by the parent layout (gap, padding). Supports horizontal and vertical orientations and an optional edge inset.'
      }
    }
  }
};
