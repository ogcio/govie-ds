 ...
 */
export const resolveResponsive = <T,>(value: ResponsiveValue<T>, toClass: (value: T, prefix: string) => string): string => {
  if (!value) {
    return '';
  }
  const entries: Partial<Record<BreakpointKey, T>> = _.isPlainObject(value) ? value as Partial<Record<BreakpointKey, T>> : {
    [Breakpoint.BASE]: value as T
  };
  return _.values(Breakpoint).filter(bp => bp in entries).map(bp => toClass(entries[bp] as T, bp === Breakpoint.BASE ? '' : `${bp}:`)).join(' ');
}