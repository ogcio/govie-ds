...
export const logosMeta = {
  ...
  argTypes: {
    ...
    label: {
      control: 'text',
      description:
        'Accessible label rendered as `aria-label`. When set, `role="img"` is applied. When empty, the logo is decorative and hidden from assistive technology.',
    },
...
export const Default = {
  ...
  play: async ({ canvasElement, step }: StoryContext<Renderer>) => {
    ...
    await step('renders standard logos', async () => {
      ...
      expect(
        canvas.getByText(/logo[\s-]?gold[\s-]?green/i),
      ).toBeInTheDocument();
    ...
    await step('renders harp logos', async () => {
      expect(
        canvas.getByText(/logo[\s-]?harp[\s-]?black/i),
      ).toBeInTheDocument();
      expect(
        canvas.getByText(/logo[\s-]?harp[\s-]?white/i),
      ).toBeInTheDocument();
    });

    await step('renders gold white logo', async () => {
      expect(
        canvas.getByText(/logo[\s-]?gold[\s-]?white/i),
      ).toBeInTheDocument();
    });
  },
};
