...
export const iconButtonMeta = {
  ...
  argTypes: {
    ...
    size: enumType(Size, { description: 'Size of the button.', defaultValue: Size.MD }),
    disabled: {
      control: false,
    ...
    ariaLabel: {
      control: false,
      description:
        'Accessible name for the button. Strongly recommended for icon-only buttons via ariaLabel, ariaLabelledBy, or a visually-hidden text child.',
    ...
    ariaHasPopup: {
      ...
      table: { type: { summary: "'menu' | 'listbox' | 'dialog' | 'grid' | 'tree' | boolean" } },
    },
    ariaControls: {
      control: false,
    ...
    ariaBusy: {
      control: false,
      description:
        'Indicates the element is being modified and assistive technologies may want to wait until changes are complete.',
      table: { type: { summary: 'boolean' } },
    },
    role: {
      control: false,
      description:
        'Overrides the implicit button role. Use only when required by composite patterns such as toolbars or menubars.',
      table: { type: { summary: 'string' } },
    },
    form: {
      control: false,
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'IconButton is a square button designed to contain only an icon. It accepts icon content via children, not icon-name props. Consumers must provide an accessible name via aria-label, aria-labelledby, or a visually-hidden text child.',
      },
    },
  },
};
...
export const Default = {
  ...
  play: async ({ canvasElement, step }: StoryContext<Renderer>) => {
    ...
    await step('has an accessible name', async () => {
      const button = canvas.getByRole('button', { name: 'Favourite' });
      expect(button).toBeInTheDocument();
    });
