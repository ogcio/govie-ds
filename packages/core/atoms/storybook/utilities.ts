 ...
 */
export function enumType<const T extends Readonly<Record<string, string>>>(
  enumObject: T,
  options?: {
    description?: string;
  ...
  return _.defaultsDeep(options?.table ? { table: options.table } : {}, base) satisfies InputType;
}
...
export const checker = (testId: string, canvas: ReturnType<typeof within>, step: StoryContext<Renderer>['step']) => ({
  exists: async (tag = 'DIV') =>
    await step(`renders ${testId} as ${tag}`, async () => {
